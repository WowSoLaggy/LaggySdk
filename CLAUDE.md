...
- `Spline.h` — header-only `Spline<T>` (aliases `SplineF`/`SplineD`/`SplineI`): a generic **Catmull-Rom** spline through a `std::vector<Vector2<T>>` of 2D control points. Passes through every point; endpoints clamp the phantom neighbours so the curve reaches its ends. Exposes `getSegmentsCount()`, `pointAt(seg, t)`/`tangentAt(seg, t)`, `sampleByStep(stepLen)` (a ~uniformly-spaced polyline), and `getControlPoints()`. Domain-neutral — no curve consumer's concepts leak in (e.g. lane offsetting belongs in the caller). Templated and `Math.h`/`Vector.h`-only, mirroring `Circle.h`.
