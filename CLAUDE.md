...
- `OrientedRect.h` — header-only `OrientedRect<T>` (aliases `OrientedRectI`/`OrientedRectF`/`OrientedRectD`): the 2D analogue of an OBB — a local axis-aligned `Rect` placed by a translation + rotation, with `getCenter()`/`containsPoint()`. `Dx::Obb::getFootprintOrientedRectXz()` builds one from a 3D box's XZ projection. `Rect.h`/`Vector.h`-only.
